...
import {
  ...
  NEWSLETTER_RESET,
  NEWSLETTER_IS_LOADING
} from './constants';
import handleError from '../../utils/error';
import { allFieldsValidation } from '../../utils/validation';
import { API_URL } from '../../constants';

export const newsletterIsLoadingg = (v) => {
  return {
    type: NEWSLETTER_IS_LOADING,
    payload: v
  }
}
...
export const subscribeToNewsletter = () => {
  return async (dispatch, getState) => {
    dispatch(newsletterIsLoadingg(true))
    try {
      const rules = {
      ...
      dispatch(newsletterIsLoadingg(false))
    }
  };
};
