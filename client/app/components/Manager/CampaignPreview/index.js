...
import { beautyCampaignTemplate } from './beautyTemplate';
 ...
 * @param {*} props
 */
const ShowBeautyCampaignTemplate = (props) => {
  const { campaign } = props;
  const templateContent = beautyCampaignTemplate(campaign);
...
const CampaignPreview = (props) => {
  ...
  if (campaign?.type === 'beauty') {
    return (
      <div className="preview-campaign">
        <ShowBeautyCampaignTemplate campaign={campaign} />
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', margin: '25px 0px' }}>
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
            ...
            />
            <label htmlFor="send_to_all_subscribers">Send to all tohanniees beauty newsletter subscribers</label>
          </div>
