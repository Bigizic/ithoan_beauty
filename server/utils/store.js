...
exports.formatOrders = async (orders) => {
  // get first name and last name of user
  // apply to the {user}
  const newOrders = [];

  for(const i in orders) {
    if (orders[i] && orders[i].cart) {
      const { firstName, lastName, email } = await User.findOne({ _id: orders[i].user });
      const newOrdersDict = {
        _id: orders[i]._id,
        total: parseFloat(Number(orders[i].total.toFixed(2))),
        created: orders[i].created,
        products: orders[i]?.cart?.products,
        currency: orders[i].cart.currency,
        status: orders[i].status,
        user: `${firstName} ${lastName} --- ${email}`,
        paymentType: orders[i].paymentType,
        amount: orders[i].amount,
      }
      newOrders.push(newOrdersDict);
    }
  }
