...
router.post('/add', async (req, res) => {
  try {
    ...
    await emailService.sendEmail(email, 'merchant-application', null);
...
router.put('/:id/active', auth, async (req, res) => {
  try {
    ...
    if (!update.isActive) {
      ...
      await emailService.sendEmail(merchantDoc.email, 'merchant-deactivate-account', null);
    }
...
const createMerchantUser = async (email, name, merchant, host) => {
  ...
  if (existingUser) {
    ...
    await emailService.sendEmail(email, 'merchant-welcome', name);
    ...
    await emailService.sendEmail(email, 'merchant-signup', {
      resetToken,
      email
    });
